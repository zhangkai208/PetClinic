...
public class AppointmentReminderServiceImpl extends ServiceImpl<AppointmentReminderMapper, AppointmentReminder>
        ...
        for (Appointment appointment : appointments) {
            try {
                ...
                if (owner != null && owner.getEmail() != null && !owner.getEmail().isEmpty()) {
                    // 检查该用户是否已经收到过此预约的提醒
                    if (hasBeenSentToUser(appointment.getId(), owner.getId())) {
                        ...
                        boolean ownerSuccess = emailService.sendSimpleMail(owner.getEmail(), ownerSubject, ownerContent);
                        // 记录发送结果（宠物主人）
                ...
                if (serviceProvider != null && serviceProvider.getUserId() != null) {
                    ...
                    if (providerUser != null && providerUser.getEmail() != null && !providerUser.getEmail().isEmpty()) {
                        ...
                        if (hasBeenSentToUser(appointment.getId(), providerUser.getId())) {
                            ...
                            String providerContent = buildProviderEmailContent(providerUser, pet, owner, appointment, serviceProvider);
                            boolean providerSuccess = emailService.sendSimpleMail(providerUser.getEmail(), providerSubject, providerContent);
                            // 记录发送结果（服务商）
                            saveReminderRecord(appointment.getId(), providerUser.getId(), providerUser.getEmail(),
                                    providerSuccess, providerSuccess ? null : "邮件发送失败", appointment.getAppointmentTime());
                            if (providerSuccess) {
                                successCount++;
                                log.info("服务商提醒发送成功：appointmentId={}, email={}", appointment.getId(), providerUser.getEmail());
                            } else {
                                log.warn("服务商提醒发送失败：appointmentId={}, email={}", appointment.getId(), providerUser.getEmail());
                            }
                        }
                    } else {
    ...
    // 记录发送结果
    private void saveReminderRecord(Long id, Long userid, String email, boolean success, String errorMessage, Date appointmentTime) {
    ...
    private String buildOwnerEmailContent(SysUser owner, Pet pet, Appointment appointment, String providerName) {
        ...
        sb.append("亲爱的 ").append(owner.getNickname() != null ? owner.getNickname() : owner.getUsername()).append("：\n\n");
        sb.append("您好！温馨提醒您，您的宠物明天有一个预约服务：\n\n");
        sb.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
        sb.append("🐾 宠物名字：").append(pet.getName()).append("\n");
        sb.append("📅 预约时间：").append(appointmentTimeStr).append("\n");
        sb.append("🏥 服务类型：").append(appointment.getServiceType() != null ? appointment.getServiceType() : "常规服务").append("\n");
    ...
    private String buildProviderEmailContent(SysUser providerUser, Pet pet, SysUser owner, 
        ...
        String ownerName = (owner != null) ? 
            (owner.getNickname() != null ? owner.getNickname() : owner.getUsername()) : "未知客户";
    ...
    /**
     * 检查某个用户是否已经收到过某个预约的提醒
     */
    private boolean hasBeenSentToUser(Long appointmentId, Long userId) {
        LambdaQueryWrapper<AppointmentReminder> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(AppointmentReminder::getAppointmentId, appointmentId)
